...
my ($species, $flank, $coverage_percent, $codon_cov_number,$RNA_sequencing_status,$bin_number,$doc,$width,$sort);
$species = "";
...
(
  ...
  'd=s'=>\$doc,
  'width=s'=>\$width,   #skip specific region to get more accurate RPKM information, AA
  'sort=s'=>\$sort         #longst or CDS_DNA
 );

if ($species eq "") {
	die "Missing one or more input file name(s)!
	use of script:
	perl step7_cov_stat.pl  -sp <species name> -d <.bg path :./ or noscreen/> -width <skip / AA>) \n"} # check parameters
...
mkdir "$doc/coverage_stat";

my %si;
open (chrom1, "/disk/ribo_seq/$species/$sort\_cDNA.chrom") or die $!;
...
sub coverage_cal                                                                               {
	...
	open (hand1, "$doc$sam\_A.bg") or die $!;
	...
	open (hand4, ">$doc/coverage_stat/$sam\_RPF_count_width$width.txt");
	...
	foreach my $id (sort keys %si)                       {
		...
		# count RPF in ORF
		for (my $i=$flank+$width*3; $i<($si{$id}-$flank-$width*3); $i+=3)              {
			# RPF inframe
			if (exists $rpf{"$id\t$i"}) {
				$inframe_pos++;                
...
sub meta_cov     {
	...
	open (hand3, ">$doc/coverage_stat/$sample\_$type\_distribution_meta.txt");
