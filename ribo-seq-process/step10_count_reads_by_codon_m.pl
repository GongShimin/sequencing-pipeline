...
sub speed                                                          {
	...
	open (hand1, "/media/hp/disk4/wzy/human_ribo_seq/$species/longest_cDNA.fa") or die $!;
	while (<hand1>) {
		$_=~ s/\s+$//;
...
###############################
	...
	open (hand6,">decode/gene_$sam\_$region\_$width\_counts.txt");
	print hand3 "\tinitiator\t".join("\t", @codon)."\n"; #join 使用VAR的值将LIST的元素组合为单个字符串，以分隔每个元素
	print hand4 "\t".join("\t", @codon)."\n";
	print hand5 "\tmedian\teffective_codon\ttotal_codon\n";
	print hand6 "gene\tlength\tread_counts\n";
	foreach my $id (keys %si)   {
		
		my $init = 0; my %cod_count; my %read_count; my $e_cod=0; my $total=0; my @everycod; my $rpkm=0;
		...
		if ($region eq "M")  {
			...
			for (my $i=$flank+$width*3; $i<($si{$id}-$flank-$width*3); $i+=1)              {
				if ( exists $rpf{"$id\t$i"} and $rpf{"$id\t$i"} > 0 )   {
					$rpkm += $rpf{"$id\t$i"};
				}
			}
		...
		if ($region eq "C")   {
			...
			if ( exists $rpf{"$id\t$i"} and $rpf{"$id\t$i"} > 0 )   {
				...
				my $cod=substr($gene{$id},$i,3);
	       			$read_count{$cod} += $rpf{"$id\t$i"};
				push(@everycod,$rpf{"$id\t$i"}) ;#将值列表推送到数组的末尾
				$cod_count{$cod} ++ ;
				$e_cod ++;
				$total=($cutoff/2)/3;   }
			}
