...
while (<REF>)   {
	...
	if ($a[7] - $a[6] > 0)   {
		if ($a[3] eq "+")    { #gene in strand + 
			...
			my $dvalue=$e_e[$e_count]-$a[6]; #取$e_cout + 1 位
			...
			if ($dvalue >= 6){ # 1 directly take the codon from genome file and record the Asite count from the bg file
				...
				$start_count = $plus{$a[2]}{$p_start} if exists $plus{$a[2]}{$p_start}; #取两次值
				$length=$a[7]-$a[6];			
						
			}elsif ($dvalue == 5 || $dvalue==4){ # 2 codon changed : ones follow by ATG , remainings in next exon ||或
				my $p_start = $a[6]+3;
				my $c1=$dvalue-3;
				my $c2=3-$c1;
			...
			my $e_count=0;
			for my $i (0..($a[8]-1)){
				if ($e_s[$i]<=($a[7]-1) && $a[7]<=$e_e[$i]){ # -1 means :end is the position(or we called index ?)+1 &&且
						$e_count=$i;
					}
				}	
