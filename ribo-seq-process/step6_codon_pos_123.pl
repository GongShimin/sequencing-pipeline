#!/usr/bin/perl
 use strict;
 use warnings;
 use Number::Format qw(round);#round 返回浮点数的四舍五入值
 use Getopt::Long;
...
sub calculate                                                          {
 ...
 # step1, get the length of each gene
    open (hand1, "/disk/ribo_seq/$species/longest_cDNA.fa") or die $!;
    while (<hand1>) {
    $_=~ s/\s+$//;
    if (/^>/)       {
    $ge=$_;
 ...
 # calculate the relative position of shift
  ...
  foreach my $id (sort keys %si)                       {
    ...
    print INFRAME join("\t",@inframe); #给数组加上\t
    print INFRAME "\n";
    ...
    undef %inframe; undef %shift1; undef %shift2; #undef 完全未定义
    print "finish $sample\n";
...
# the info: start position # region covered # $chromosome # strand # bin-size # hash for plus # hash for minus
  ...
  for (my $i=0+$flank+$ex*3; $i<=$range-$flank-$ex*3; $i+=$bin)  {   
    ...
    for (my $j=0; $j<$bin; $j++)  {
      my $pos=$j+$i; 
      $count+=$$ref{$chr."\t".$pos} if exists $$ref{$chr."\t".$pos} ; #我不理解
                          }
                           
  push @result, $count;      }  #push把List中的值压入列表的末尾
